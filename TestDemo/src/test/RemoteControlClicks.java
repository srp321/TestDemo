...
public class RemoteControlClicks {
	...
	public static int getMinimumClicks(String startEnd,String channelsString){
		...
		minClickCount+=minimum(Math.abs(channels[0]-start),
						digitCount(channels[0]),
						Math.abs(channels[0]-end));
		int previousChannel = start;
		for(i=1;i<channels.length;i++){
			minClickCount+=minimum(Math.abs(channels[i]-channels[i-1]),
							digitCount(channels[i]),
